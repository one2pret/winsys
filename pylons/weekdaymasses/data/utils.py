...
def church_link (request, language, area=None, church=None, area_code=None, church_id=None):
  ...
  return url (request, u"%s/churches#p%d" % (base_link, church_id or church.id))
...
def filter_search_words (phrase):
  ...
  words = [w.strip () for w in splitter.split (phrase.lower ())]
  return [w for w in words if len (w) >= 3 and w not in words_to_exclude]
